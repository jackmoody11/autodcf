...
class DCF(AbstractDCF):
    ...
    def forecast(self):
        ...
        self._forecast['Period'] = np.arange(-1, self.window + 1)
        self._forecast['Year'] = np.arange(datetime.now().year, datetime.now().year + self.window + 1)
        ...
        self._forecast['EBIT'] = self._forecast['EBITDA'] - self._forecast['D&A']
        self._forecast['Interest'] = multiply_by_sales_percent(self._forecast, self._interest_to_sales)
        ...
        self._forecast['Change in NWC'] = np.diff(self._forecast['Sales']) * self._change_in_nwc_to_change_in_sales
        tax_rates = (self._forecast['Taxes'] / self._forecast['EBT'])
        self._forecast['Free Cash Flow'] = self._forecast['EBITDA'] * tax_rates + self._forecast['D&A'] * tax_rates - \
                                           self._forecast['Capex'] - self._forecast['Change in NWC']
        self._forecast['Discounted Free Cash Flow'] = discount_cash_flows(self._forecast['Free Cash Flow'],
                                                                          self._discount_rate)
